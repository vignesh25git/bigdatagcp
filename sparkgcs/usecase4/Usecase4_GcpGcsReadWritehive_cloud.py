...
def main():
    ...
    gcs_df = spark.read.csv("gs://bigdatavicky1983/dataset/custs",mode='dropmalformed',schema=custstructtype1)
    gcs_df.show(10)
    ...
    print(curts)
    gcs_df.repartition(2).write.json("gs://bigdatavicky1983/dataset/cust_output_json_"+curts) #change to your cloud bucket location
    ...
    curts = spark.createDataFrame([1], IntegerType()).withColumn("curts", current_timestamp()).select(date_format(col("curts"), "yyyyMMddHHmmSS")).first()[0]
    print(curts)
    gcs_df.repartition(2).write.mode("overwrite").csv("gs://bigdatavicky1983/dataset/cust_csv")  #change to your cloud bucket location
    print("gcs Write Completed Successfully")
